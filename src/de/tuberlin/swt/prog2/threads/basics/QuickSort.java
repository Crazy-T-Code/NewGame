...
public class QuickSort extends Thread {
	...
	public static void quickSortMultithreaded(float[] toSort) {
		int pivot = partition(toSort, toSort[toSort.length / 2], 0,
		        toSort.length - 1);
	...
	/**
	 * Partitions the values between indices left and right in given float array
	 * according to the given pivot value. Returns the index of the pivot
	 * element (last index of the lower sub array).
	 */
	private static int partition(float[] toSort, float pivot, int left,
	        int right) {
